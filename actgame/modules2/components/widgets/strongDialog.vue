<template>
<div id='modal-template'  class='dialog wrap' v-show='isShowDlgFlag'>
    <div class='strong-dlg-shadow'></div>
    <div id='strong-dlg-body' v-bind:class="[ 'dlg-body', bodyClass ? bodyClass : '']" >
        <h1 v-if='isShowTitleFlag' v-html="titleText"></h1>
        ...
        <ul class='strong-dlg-ul'>
            <li v-for='(btnItem,index) in btnsObj' class='strong-dlg-li'>
                <a v-bind:class="[ 'strong-dlg-a', 'dlg-submit-btn', btnItem.btnClass ? btnItem.btnClass : '' ]" @click='onBtnClick(index)'>{{ btnItem.btnText }}</a>
            </li>
        </ul>
    </div>
</div>
...
export default {
    ...
    methods: {
        showStrongDialog: function(dlgMsgObj){
            ...
            if( this.bodyClass ){
                $("#modal-template #strong-dlg-body").addClass( "" + this.bodyClass );
            }

//            if( ){
//                $("#modal-template #strong-dlg-body").addClass( '' +  );
//            }
